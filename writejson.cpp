...
void WriteJson::WriteData( QStringList data )
{
    ...
    QString line_0("time, seq, imei, timestamp, car_status,net_state,bat,nb_ver,ver,nb_state\n");

    QFile csvFile("test.csv");
    if (csvFile.open(QIODevice::ReadWrite | QIODevice::Truncate ))
    {
        csvFile.write(line_0.toUtf8());
        for( int j = 0; j < data.length(); j++)
        {
           ...
           // qDebug() << sections.at(9).trimmed() << endl;
            ...
            QString time = sections.at(2);
            QString time2 = sections.at(3);
            QString seq =  sections_part.at(2);
            QString imei =  sections_part.at(1);
            csvFile.write(time.toUtf8() +" " + time2.toUtf8() +","+seq.toUtf8()+","+"'"+imei.toUtf8()+"\n");
